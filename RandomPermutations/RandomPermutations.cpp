// RandomPermutations.cpp
// Created 8/31/2021 10:07:46 AM
...
void report(const std::vector<int>& a, int runs) {
	...
	std::cout << "1,2,3: " << a[0] << "   (" << (100.0 * a[0]) / runs << "%)\n";
	std::cout << "1,3,2: " << a[1] << "   (" << (100.0 * a[1]) / runs << "%)\n";
	std::cout << "2,1,3: " << a[2] << "   (" << (100.0 * a[2]) / runs << "%)\n";
	std::cout << "2,3,1: " << a[3] << "   (" << (100.0 * a[3]) / runs << "%)\n";
	std::cout << "3,1,2: " << a[4] << "   (" << (100.0 * a[4]) / runs << "%)\n";
	std::cout << "3,2,1: " << a[5] << "   (" << (100.0 * a[5]) / runs << "%)\n";
}
...
int main() {
	...
	report(permutation_tally, RUNS);   // Report results
	...
	for (int i = 0; i < RUNS; i++) {  // Run 1,000,000 times
		// Make a copy of the original vector
		working = original;
		// Permute the vector with the standard shuffle algorithm
		std::shuffle(working.begin(), working.end(), rng);
		// Count this permutation
		permutation_tally[classify(working)]++;
	}
	report(permutation_tally, RUNS);   // Report results
